...
class ADCNet(nn.Module):
	def __init__(self, num_of_adc_p, num_samples_L_tilde, num_code_words, max_labels, max_samples):
		...
		self.analog_network = AnalogNetwork(num_of_adc_p)
		self.sampling_layer = SamplingLayer(num_of_adc_p, num_samples_L_tilde)
		self.quantization_layer = QuantizationLayer(num_code_words, max_labels, max_samples)
		self.digital_network = DigitalNetwork(num_of_adc_p, num_samples_L_tilde)
...
def train(detector, parameters, train_opt_data):
	...
	for r, ro in enumerate(np.flip(snr_vals)):
		...
		num_code_words = 2.0 ** parameters['num_code_words']
		...
		net = ADCNet(int(round(num_of_adc_p)), int(round(num_samples_L_tilde)), int(round(num_code_words)),
		             np.max(labels_train), np.max(train_samples))
...
def evaluate(parameters, nets):
	...
	print("BER: ", BER)
	return 0.7 * BER + 0.3 * (0.1 * overall_bits)
	...
	for r, ro in enumerate(np.flip(snr_vals)):
		...
		with torch.no_grad():
			for i, data in tqdm(enumerate(DataLoader(test_set, batch_size=len(test_samples)))):
				...
				est_ber = np.floor((outputs.numpy()[:, None] % (2 * power)) / power).T
				est_ber_scale = 2 * est_ber - np.ones(test_data.shape)
				BER.append(np.mean(np.mean(est_ber_scale != test_data, axis=0)))
